# Добавляем рецепт: стабильный сигил (через Avaritia верстак)
...
# Исправляем дублирование рецепта (занавесы из ExtraUtlities и занавесы из MalisisDoors)
 recipes.remove(<ExtraUtilities:curtains>);
 recipes.addShapeless(<ExtraUtilities:curtains>,
  [<malisisdoors:item.curtain_black>]);

# Добавляем вспомогательные описания к Карьеру Края/Термической помпе Края в случае, если они работают только в приватных регионах (фикс от gamerforea)
 # Карьер Края
...
# Добавляем вспомогательное описание к QED, если отключено получение х3 слитков с руды
